...
| `dependabot-triage.sh` | Audits open PRs and merges the ones you name. `--audit` (default) is read-only and prints a verdict per PR; `--merge <pr>` re-verifies then squash-merges. Checks what the ruleset cannot: Dependabot identity by user id, every commit's authorship *and* signature, changed paths against a per-ecosystem allowlist, and that each added action pin is a real released tag. Refuses a major without `--notes-reviewed`. The `merge-dependabot-prs` skill wraps it. |
